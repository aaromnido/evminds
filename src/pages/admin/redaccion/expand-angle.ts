...
/**
 * POST /admin/redaccion/expand-angle
 *
 * Admin-gated proxy to the `editorial-expand-angle` Edge Function.
 *
 * Body: { title, angle }
 * Response: { ok: true, angle } | { error }
 */
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    const res = await fetch(`${supabaseUrl}/functions/v1/editorial-expand-angle`, {
      method: "POST",
      headers: {
    ...
    console.error("expand-angle proxy error:", err);
